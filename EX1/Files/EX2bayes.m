...
% Configuration:
alg1 = 'traingda';% First training algorithm to use
...
dx=0.003;% Decrease this value to increase the number of data points
x=0:dx:3*pi;y=sin(x.^2);
a13=sim(net1,x); a23=sim(net2,x);

mse_test_a13 = 1/size(x,2)*sum((y - a13).^2);
mse_test_a23 = 1/size(x,2)*sum((y - a23).^2);

fprintf('The mse_test_a13 of method %s has training error: %d.\n',alg1,round(mse_test_a13,3));
